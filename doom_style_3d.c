...
typedef struct CollisionData {
  v2 offset;  // adjusting position by this offset makes the object only touch
              // and not overlap
  bool didCollide;
} CollisionData;
...
void renderDirSprite(DirectionalSprite *dSprite, v2 pos, v2 size,
                     double height);
...
int main(int argc, char *argv[]) {
  ...
  window =
      SDL_CreateWindow("Doom style 3D!", SDL_WINDOWPOS_CENTERED,
...
Enemy *createEnemy(v2 pos) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    enemy->dirSprite->sprites[i]->texture =
        make_texture(renderer, fileWithExtension);
  }
...
void init() {  // #INIT
  ...
  floorAndCeiling = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
  ...
  leftHandSprite->animations[0]->frames[0] =
      make_texture(renderer, "Textures/rightHandAnim/rightHandAnim5.bmp");
  leftHandSprite->animations[1] = create_animation(5);
  leftHandSprite->animations[1]->frames = malloc(sizeof(SDL_Texture *) * 5);
  getTextureFiles("Textures/rightHandAnim/rightHandAnim", 5,
                  &leftHandSprite->animations[1]->frames);
  ...
  getTextureFiles("Textures/ShootEffectAnim/shootHitEffect", 5,
                  &shootHitEffectFrames);
...
void handle_input(SDL_Event event) {
  switch (event.type) {
    ...
    case SDL_MOUSEMOTION:
      ...
      player->handOffset.x =
          lerp(player->handOffset.x, -event.motion.xrel * 2, 0.06);
...
v2 get_key_vector(SDL_Keycode k1, SDL_Keycode k2, SDL_Keycode k3,
                  SDL_Keycode k4) {
...
void checkCollisions() {
  ...
  for (int i = 0; i < 1; i++) {
    RayCollisionData *result = castRay(
        v2_add(player->pos, v2_mul(dirs[i], to_vec(player->collSize))), velDir);
    if (result != NULL && v2_distance_squared(player->pos, result->collpos) <
                              player->collSize * player->collSize) {
...
void playerTick(u64 delta) {
  ...
  if (!v2_equal(keyVec, to_vec(0))) {
    double t =
        sin(mili_to_sec(SDL_GetTicks64()) * (15 + (int)player->sprinting * 5)) *
        3;
    player->height = t;
  ...
  player->vel = v2_lerp(player->vel,
                        v2_add(v2_mul(move_dir, to_vec(keyVec.x)),
                               v2_mul(move_dir_rotated, to_vec(keyVec.y))),
                        0.15);
...
void chaserTick(Chaser *chaser, u64 delta) {
  ...
  if (data == NULL ||
      v2_distance_squared(pos, data->collpos) <
          dist * dist) {  // something is between the chaser and the player
    chaser->seeingPlayer = false;
  } else {
...
void tick(u64 delta) {
  ...
  if (isCameraShaking) {
    ...
    if (cameraShakeTimeToNextTick <= 0) {
      ...
      if (cameraShakeTicksLeft <= 0) {
        ...
        v2 rawShake = {randf_range(-cameraShakeCurrentStrength,
                                   cameraShakeCurrentStrength),
                       randf_range(-cameraShakeCurrentStrength,
                                   cameraShakeCurrentStrength)};
        cameraOffset = v2_mul(
            rawShake, to_vec((double)cameraShakeTicksLeft / cameraShakeTicks));
      }
    }
  }
  ...
  animationTick(
      animatedWallSprite->animations[animatedWallSprite->currentAnimationIdx],
      delta);
...
void renderDebug() {  // #DEBUG
  ...
  SDL_Rect playerDirRect = {
      player->pos.x + v2_rotate_to((v2){10, 0}, deg_to_rad(player->angle)).x -
          3,
      player->pos.y + v2_rotate_to((v2){10, 0}, deg_to_rad(player->angle)).y -
          3,
      6, 6};
...
v2 screenToFloor(v2 pos) {
  ...
  double wallSize =
      abs(pos.y - WINDOW_HEIGHT / 2) * 2;  // size of a wall at that position
  if (wallSize == 0) {
    return to_vec(0);
  }
...
void calcFloorAndCeiling() {
  ...
  for (int row = RESOLUTION_Y / 2; row < RESOLUTION_Y; row++) {
    ...
    for (int col = 0; col < RESOLUTION_X; col++) {
      ...
      if (in_range(tileRow, 0, TILEMAP_HEIGHT - 1) &&
          in_range(tileCol, 0, TILEMAP_WIDTH - 1)) {
      ...
      Pixel floor_pixel = TextureData_get_pixel(
          floorTex, loop_clamp(point.x, 0, 36), loop_clamp(point.y, 0, 36));

      floor_pixel.r *= light;
      floor_pixel.g *= light;
      floor_pixel.b *= light;

      int floor_pixel_i = floor_pixel.r << 24 | floor_pixel.g << 16 |
                          floor_pixel.b << 8 | floor_pixel.a;
      ...
      if (ceilingTile == -1) {
        ...
        ceil_pixel = TextureData_get_pixel(
            ceilingTex, loop_clamp(point.x, 0, 36), loop_clamp(point.y, 0, 36));
        ceil_pixel.r *= light;
        ceil_pixel.g *= light;
        ceil_pixel.b *= light;
      }

      int ceil_pixel_i = ceil_pixel.r << 24 | ceil_pixel.g << 16 |
                         ceil_pixel.b << 8 | ceil_pixel.a;
...
void renderTexture(SDL_Texture *texture, v2 pos, v2 size, double height) {
  double cosAngleToForward =
      v2_cos_angle_between(playerForward, v2_sub(pos, player->pos));
  ...
  double fovFactor = tanHalfStartFOV / tanHalfFOV;
  double finalSize =
      WALL_HEIGHT * WINDOW_HEIGHT / dist * fovFactor / cosAngleToForward;
  v2 finalSizeVec = (v2){finalSize * size.x / 100, finalSize * size.y / 100};

  SDL_Rect dstRect = {screenFloorPos.x - finalSizeVec.x / 2 + cameraOffset.x,
                      screenFloorPos.y - finalSizeVec.y / 2 -
                          (height * 100 / dist) - player->height +
                          cameraOffset.y,
                      finalSizeVec.x, finalSizeVec.y};
...
void renderDirSprite(DirectionalSprite *dSprite, v2 pos, v2 size,
                     double height) {
  SDL_Texture *texture =
      getSpriteCurrentTexture(getDSpriteCurrentSprite(dSprite, pos));
...
RenderObject *getWallStripe(int i) {
  ...
  double cosAngleToForward = v2_cos_angle_between(rayDir, playerForward);
  double collDist =
      v2_distance(data->startpos, data->collpos) * cosAngleToForward;
  double fovFactor = tanHalfStartFOV / tanHalfFOV;
...
int addWallStripes_Threaded(void *data) {
  int idx = *(int *)data;
  for (int i = RESOLUTION_X / NUM_WALL_THREADS * idx;
       i < RESOLUTION_X / NUM_WALL_THREADS * (idx + 1); i++) {
...
arraylist *getRenderList() {
  ...
  for (int i = 0; i < NUM_WALL_THREADS; i++) {
    indicies[i] = i;
    threads[i] =
        SDL_CreateThread(addWallStripes_Threaded, "thread wall", &indicies[i]);
  }
  for (int i = 0; i < NUM_WALL_THREADS; i++) {
  ...
  for (int i = 0; i < RESOLUTION_X; i++) {
    SortObject sObj = {wallStripesToRender[i],
                       wallStripesToRender[i] == NULL
                           ? 9999999999
                           : wallStripesToRender[i]->dist_squared};
    sortObjects[i] = sObj;
  }
...
void renderHUD() {
  SDL_Rect leftHandRect = {player->handOffset.x + cameraOffset.x,
                           player->handOffset.y + cameraOffset.y, WINDOW_WIDTH,
                           WINDOW_HEIGHT};
  ...
  double redness =
      pow(0.5, (int)(max(player->shootChargeTimer, player->pendingShots) *
                     3));  // 0.5 0.25 0.125 0.075
  if (redness == 1)
    redness = 0;
  else
  ...
  SDL_Rect crosshairRect = {WINDOW_WIDTH / 2 - 8, WINDOW_HEIGHT / 2 - 8, 16,
                            16};
  ...
  SDL_Rect playerPendingShotsRect = {WINDOW_WIDTH / 2 + -10 * shots,
                                     WINDOW_HEIGHT * 0.8, 20 * shots,
                                     WINDOW_HEIGHT * 0.05};
...
void drawSkybox() {
  ...
  double x =
      loop_clamp(player->angle / fov * WINDOW_WIDTH, 0, WINDOW_WIDTH);
    ...
    switch (rObj->type) {
      ...
      case (int)ENEMY:;
        ...
        if (enemy->dirSprite != NULL) {
          renderDirSprite(enemy->dirSprite, enemy->entity->pos,
                          enemy->entity->size, enemy->entity->height);
        } else {
      ...
      case (int)BULLET:;
        ...
        if (bullet->dirSprite != NULL) {
          renderDirSprite(bullet->dirSprite, bullet->entity->pos,
                          bullet->entity->size, bullet->entity->height);
        } else {
...
RayCollisionData *castRay(v2 pos, v2 dir) {
  ...
  v2 scalingVec = {sqrt(1 + (dir.y / dir.x) * (dir.y / dir.x)),
  ...
  while (!found && dist < maxDist) {
    ...
    if (in_range((int)currentCell.y, 0, TILEMAP_HEIGHT - 1) &&
        in_range((int)currentCell.x, 0, TILEMAP_WIDTH - 1)) {
  ...
  if (found) {
    ...
    if (lastStepDir.x != 0) {
      data->collIdx =
          (data->collpos.y - floor(data->collpos.y / tileSize) * tileSize) /
          tileSize;
    } else {
      data->collIdx =
          (data->collpos.x - floor(data->collpos.x / tileSize) * tileSize) /
          tileSize;
    }
...
RayCollisionData *ray_circle(Raycast ray, CircleCollider circle) {
  if (v2_distance(ray.pos, circle.pos) <= circle.radius ||
      v2_dot(ray.dir, v2_dir(ray.pos, circle.pos)) < 0) {
    return NULL;
  }
  ...
  // raypos + raydir * (a - d)
  v2 collision_pos =
      v2_add(ray.pos, v2_mul(ray.dir, to_vec(a - d)));  // woohoo!
...
void load_level(char *file) {
  ...
  char *data = malloc(sizeof(char) *
                      fileSize);  // sizeof char is 1 but i do it for clarity
  ...
  for (int r = 0; r < TILEMAP_HEIGHT; r++) {
    for (int c = 0; c < TILEMAP_WIDTH; c++) {
      ...
      v2 tileMid =
          v2_add(v2_mul((v2){c, r}, to_vec(tileSize)), to_vec(tileSize / 2));
...
void playerShoot() {
  ...
  RayCollisionData *entityRayData =
      castRayForEntities(player->pos, playerForward);
  ...
  if (entityRayData != NULL) {
    entitySquaredDist =
        v2_distance_squared(player->pos, entityRayData->collpos);
  }
  if (wallRayData != NULL) {
  ...
  Effect *hitEffect =
      createEffect(effectPos, to_vec(35), createSprite(true, 1), 1);
...
void enemyTick(Enemy *enemy, u64 delta) {
  ...
  RayCollisionData *rayData =
      castRayForAll(v2_add(enemy->entity->pos, v2_mul(dir, to_vec(15))), dir);

  enemy->seeingPlayer =
      rayData != NULL && (Player *)rayData->collider == player;
...
Sprite *getDSpriteCurrentSprite(DirectionalSprite *dSprite, v2 spritePos) {
  ...
  if (dSprite->dirCount < 2) {
    ...
    double relativeAngle = rad_to_deg(v2_get_angle(dSprite->dir)) +
                           rad_to_deg(playerAngleToSprite);
    double min = 9999999;
    int minIdx = -1;
    for (int i = 0; i < dSprite->dirCount; i++) {
      double angle = (360.0 / dSprite->dirCount) * i;
      double aDist =
          angleDist(angle, relativeAngle);  // ANGLE DIST WORKS CORRECTLY 100%
...
bool intersectCircles(CircleCollider c1, CircleCollider c2) {
  return v2_distance_squared(c1.pos, c2.pos) <
         (c1.radius + c2.radius) *
             (c1.radius + c2.radius);  // dist^2 < (r1 + r2)^2
}
...
void bulletTick(EnemyBullet *bullet, u64 delta) {
  ...
  bullet->entity->pos =
      v2_add(bullet->entity->pos, v2_mul(bullet->dir, to_vec(bullet->speed)));
  bullet->collider->pos = bullet->entity->pos;
...
void shooterEnemyShoot(ShooterEnemy *shooter) {
  EnemyBullet *bullet =
      createDefaultBullet(shooter->enemy->entity->pos, shooter->enemy->dir);
  if (bullet == NULL) {
...
CollisionData getCircleTileMapCollision(CircleCollider circle) {
  ...
  gridCheckEnd.x = (int)((circle.pos.x + circle.radius) / tileSize) +
                   2;  //+ 2 to account for rounding up
  ...
  for (int row = gridCheckStart.y; row < gridCheckEnd.y; row++) {
    for (int col = gridCheckStart.x; col < gridCheckEnd.x; col++) {
      if (!in_range(row, 0, TILEMAP_HEIGHT - 1) ||
          !in_range(col, 0, TILEMAP_WIDTH - 1))
        continue;
      if (levelTileMap[row][col] == -1) continue;
      CollisionData data =
          getCircleTileCollision(circle, (v2){col * tileSize, row * tileSize});
      if (data.didCollide) {
        result.didCollide = true;
